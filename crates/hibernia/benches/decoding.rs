...
pub fn decoding_benchmark(c: &mut Criterion) {
    ...
    let nl2_buffer = fs::read(root.join("data/NL2_Sony_H.jsv")).expect("can't read NL2_Sony_H.jsv");
    ...
    let caba3_buffer = fs::read(root.join("data/CABA3_SVA_B.264")).expect("can't read CABA3_SVA_B.264");
    ...
    let sva_ba2_buffer = fs::read(root.join("data/SVA_BA2_D.264")).expect("can't read SVA_BA2_D.264");
