...
mod tests {
    ...
    fn test_poc_type2_non_ref() {
        ...
        // IDR
        decoder.poc_state.calculate_poc(&slice, ReferenceDisposition::Idr);

        // Frame 1 (Ref)
        slice.header.frame_num = 1;
        decoder.poc_state.calculate_poc(&slice, ReferenceDisposition::NonIdrReference);
        ...
        let poc = decoder.poc_state.calculate_poc(&slice, ReferenceDisposition::NonReference);
        // tempPOC = 2 * (0 + 2) - 1 = 3
        assert_eq!(poc, 3);

        // Decoder state should NOT update for non-ref
        assert_eq!(decoder.poc_state.prev_frame_num, 1);
    }
    ...
    fn test_poc_type2_wrapping() {
        ...
        let poc = decoder.poc_state.calculate_poc(&slice, ReferenceDisposition::NonIdrReference);
        assert_eq!(poc, 30); // 2 * 15
        assert_eq!(decoder.poc_state.prev_frame_num, 15);
        ...
        // POC = 2 * (16 + 0) = 32.
        assert_eq!(poc, 32);
        assert_eq!(decoder.poc_state.prev_frame_num, 0);
        assert_eq!(decoder.poc_state.prev_frame_num_offset, 16);
